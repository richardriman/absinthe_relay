...
defmodule Absinthe.Relay.Connection do
  ...
  if Code.ensure_loaded?(Ecto) do
    ...
    @spec from_query(
            Ecto.Queryable.t(),
            (Ecto.Queryable.t() -> [term]) | {module(), [term]},
            Options.t(),
            from_query_opts
          ) :: {:ok, map} | {:error, any}
  ...
  if Code.ensure_loaded(Paginator) do
    ...
    defp cursor_pagination_from_query(query, repo_term, args, opts) do
      with {:ok, cursor_fields} <- cursor_fields(opts),
           {:ok, _direction, limit} <- __MODULE__.limit(args, opts[:max]),
           {:ok, cursor} <- cursor(args) do
        {entries, page_info} = paginate(query, repo_term, args, cursor, cursor_fields, limit)
        edges = entries |> Enum.map(&make_edge(&1, args, cursor_fields))
    ...
    defp paginate(query, repo_term, %{first: f}, cursor, cursor_fields, _limit) do
      {records, more_pages} = records_until_cursor(query, repo_term, f, cursor_fields, cursor)
    ...
    defp paginate(query, repo_term, %{last: l}, cursor, cursor_fields, _limit) do
      {records, more_pages} =
        query
        |> last()
        |> exclude(:limit)
        |> records_until_cursor(repo_term, l, cursor_fields, cursor)
    ...
    defp records_until_cursor(query, {repo_module, repo_opts}, count, cursor_fields, cursor) do
  ...
  defp cursor(%{after: cursor}) when not is_nil(cursor), do: {:ok, after: cursor}

  defp cursor(%{before: cursor}) when not is_nil(cursor), do: {:ok, before: cursor}
